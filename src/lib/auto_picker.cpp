...
#define AUTO_COUNT 0 // 'NONE' is zero, so it's the count of actual variations
const char * autos[] = {
...
void autoPickerRun() {
  ...
  switch (stage) {
    ...
    case (STAGE_SIDE): {
      ...
      if (lcd.btn_l_new || lcd.btn_r_new) {

        // update side variable
        if (lcd.btn_l_new) auto_side = LEFT;
        if (lcd.btn_r_new) auto_side = RIGHT;
    ...
    case (STAGE_COLOR): {
      ...
      else if (lcd.btn_l_new || lcd.btn_r_new) {

        // update side variable
        if (lcd.btn_l_new) auto_color = BLUE;
        if (lcd.btn_r_new) auto_color = RED;
    ...
    case (STAGE_VARIATION): {
        ...
        // go left one option
        if (lcd.btn_l_new && auto_variation >= 1) {
          auto_variation--;
          lcd.setText(0, autos[auto_variation]);
        }
        ...
        // go right one option
        if (lcd.btn_r_new && auto_variation < AUTO_COUNT) {
          auto_variation++;
          lcd.setText(0, autos[auto_variation]);
        }
